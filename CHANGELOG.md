...
- **`python -m xstate_statemachine`** runs the CLI (a top-level
  `__main__.py`; `python -m xstate_statemachine.cli` still works). On
  Windows machines governed by Application Control / AppLocker / Smart App
  Control, pip's unsigned `xsm.exe` launcher is refused ("An Application
  Control policy has blocked this file") while `python.exe` is trusted;
  the module spelling is the supported way around it, `xsm info` prints
  it on an `Also run as:` line, and the CLI guide documents a verified
  `xsm.cmd` shim recipe. The launcher is generated by pip on the user's
  machine, so the package cannot sign it.
