...
- **Improved `SyncInterpreter` Shutdown**: The `stop()` method of the `SyncInterpreter` now ensures a more orderly shutdown by explicitly iterating and stopping all child actors _before_ canceling any active `after` timers. This enhances reliability and resource management for complex synchronous systems.
...
- Removed special treatment for 'spawn\_' actions in the logic loader (`LogicLoader`), now treating them as regular actions. This may require users to adjust bindings for actor spawning in configurations.
...
- **Factory** `create_machine()` gains `mode="async" | "sync"` (default _async_).
