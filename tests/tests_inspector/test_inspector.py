...
# For now, adding more tests to the main class provides more value.
    ...
    def test_websocket_command_pause_resume(self, mock_start_server):
        ...
        from src.xstate_statemachine.inspector.server import active_interpreters
        ...
        with self.client.websocket_connect("/ws") as websocket:
            # Test Pause
            websocket.send_json({"command": "pause", "machine_id": "test_ws_cmd"})
            # Give a moment for the server to process
            time.sleep(0.01)
            interpreter.pause.assert_called_once()

            # Test Resume
            websocket.send_json({"command": "resume", "machine_id": "test_ws_cmd"})
            time.sleep(0.01)
    ...
    def test_websocket_invalid_command(self, mock_start_server):
        ...
        with self.client.websocket_connect("/ws") as websocket:
            websocket.send_json({"command": "invalid_command", "machine_id": "test_ws_cmd"})
    ...
    @patch('src.xstate_statemachine.inspector.plugin.start_inspector_server')
    def test_websocket_command_for_nonexistent_machine(self, mock_start_server):
        """Test that a command for a non-existent machine is ignored."""
        with self.client.websocket_connect("/ws") as websocket:
            websocket.send_json({"command": "pause", "machine_id": "nonexistent"})
...
class TestInspectorPluginPayloads(unittest.TestCase):
    ...
    def test_on_action_execute_payload(self, mock_start_server):
        ...
        with patch.object(self.plugin, '_handle_inspection_event') as mock_handle:
    ...
    def test_on_guard_evaluated_payload(self, mock_start_server):
        ...
        with patch.object(self.plugin, '_handle_inspection_event') as mock_handle:
            self.plugin.on_guard_evaluated(self.mock_interpreter, "my_guard", event, True)
            mock_handle.assert_called_once()
            args, _ = mock_handle.call_args
    ...
    def test_on_service_done_payload(self, mock_start_server):
        ...
        with patch.object(self.plugin, '_handle_inspection_event') as mock_handle:
            self.plugin.on_service_done(self.mock_interpreter, invoke_def, result)
            mock_handle.assert_called_once()
            args, _ = mock_handle.call_args
    ...
    def test_on_service_error_payload(self, mock_start_server):
        ...
        with patch.object(self.plugin, '_handle_inspection_event') as mock_handle:
            self.plugin.on_service_error(self.mock_interpreter, invoke_def, error)
            mock_handle.assert_called_once()
            args, _ = mock_handle.call_args
