...
| `strict_config` | `Optional[bool]` | No | `None` | **[0.8.1]** (#216) `True` refuses an unrecognised **top-level** config key with `InvalidConfigError` (a misspelled `actionErrorPolicyy` / `onUnhandledEvent` / `Strict` otherwise passes a clean build and the policy silently reverts to its permissive default). `None` reads the config's own `strictConfig` key, else `False`: unknown keys are logged at WARNING with a "did you mean" hint. `x-`-prefixed keys and `meta` / `description` / `tags` / `version` are always accepted. |
