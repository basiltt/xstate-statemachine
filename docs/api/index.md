...
| `strict_config` | `Optional[bool]` | No | `None` | **[0.8.1]** (#216, #220) `True` refuses an unrecognised config key — at the root **and in every state, transition and invoke** — with `InvalidConfigError` naming the path (a misspelled `actionErrorPolicyy` / `onUnhandledEvent` / `Strict` otherwise passes a clean build and the policy silently reverts to its permissive default). `None` reads the config's own `strictConfig` key, else `False`: unknown keys are logged at WARNING with a "did you mean" hint. `x-`-prefixed keys and `meta` / `description` / `tags` / `version` are always accepted. |
...
| `.send_threadsafe(event, *, internal=None, **payload)` | `(Union[str, Dict, Event, DoneEvent, AfterEvent, ErrorEvent], Optional[bool], **Any) -> concurrent.futures.Future[None]` | `concurrent.futures.Future[None]` | Sends an event from **any** thread by routing the enqueue through the interpreter's owning event loop. Returns a `Future` you may `.result()` on to block until the event is queued (not processed). **[0.8.1]** Under a bounded inbox with `OverflowPolicy.RAISE`, a full inbox raises `QueueOverflowError` **on the calling thread** (#157) — backpressure at the call site, not on a future a fire-and-forget producer never reads. That call-site check is optimistic: a concurrent producer may still be refused **on the loop**, in which case the returned future carries the `QueueOverflowError` *and* — so a fire-and-forget producer's shed rate is never hidden — the interpreter logs a WARNING and fires `on_event_dropped(reason="queue_full")` (#157 reopen). `internal` (#150): `None` classifies the send by context — issued from inside one of this interpreter's own actions (or a thread/executor that inherited that `contextvars` context) it is a self-send charged to `maxIterations`; a plain `threading.Thread` does *not* inherit the context, so an action handing its own re-trigger to one must pass `internal=True` (or start the thread with `contextvars.copy_context().run`). |
...
| `.chain_trips` | `int` | **[0.8.1]** Monotonic count of chain-budget and settle-budget trips since construction (#222). Never resets; a supervisor samples it at any interval and diffs. |
| `.last_chain_error` | `Optional[BaseException]` | **[0.8.1]** The `RunawayChainError` of the most recent trip, latched: it survives every later event and is cleared only by `.clear_chain_error()` (#222). |
...
| `.clear_chain_error()` | `() -> None` | `None` | **[0.8.1]** Per `Interpreter.clear_chain_error()` (#222). |
...
| `ReentrantWaitError` **[0.8.1]** | An action awaited `send(..., wait=True)` on its **own** interpreter (#219). The receipt resolves only when the run loop processes the event, and the loop cannot advance until the action returns — a deadlock, refused eagerly at the call site. Hand the receipt out and await it *later*, or send without `wait`. The sync engine raises it for the same shape (its receipt would describe the running step, not the event's). | `async def act(i, c, e, a): await i.send("GO", wait=True)` as an `entry` action. |
