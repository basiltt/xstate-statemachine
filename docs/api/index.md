...
| `.chain_trips` | `int` | **[0.9.0]** Monotonic count of chain-budget and settle-budget trips since construction (#222). Never resets; a supervisor samples it at any interval and diffs. |
| `.last_chain_error` | `Optional[BaseException]` | **[0.9.0]** The `RunawayChainError` of the most recent trip, latched: it survives every later event and is cleared only by `.clear_chain_error()` (#222). |
