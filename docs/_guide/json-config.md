...
- **At build time (0.9.0).** If the raised event is a *literal* — a string or a `{"type": ...}` dict — `create_machine()` verifies it against `known_events` and raises `InvalidConfigError` (with a *Did you mean …?* suggestion) when nothing handles it. A typo in your config is a configuration error and should never reach runtime.
