...
| Where | Keys |
|---|---|
| 🏠 **Top level** | `id` · `initial` · `context` · `states` · `strict` · `strictTargets` · `strictConfig` · `maxIterations` · `spawnBlockingTimeout` · `actionErrorPolicy` · `guardErrorPolicy` · `onUnhandled` · `output` · `meta` · `description` · `tags` · `version` · any `x-…` key. **Unknown keys** (#216): logged at WARNING with a "did you mean" hint by default; refused with `InvalidConfigError` under `strictConfig: true` or `create_machine(..., strict_config=True)`. A misspelled policy key otherwise silently reverts to its default — `actionErrorPolicyy` → `continue`, `Strict` → `false`. |
