...
> - **Timers are NOT resumed by default** — a snapshot records that an `after` timer was pending, not how far along it was. After a static restore `interpreter.has_dormant_timers` is `True`. Pass `from_snapshot(..., restart_timers=True)` (defaults to the value of `restart_services`) to have `start()` re-arm every dormant timer **from zero** (0.9.0, #128). A timer that had already *fired* and was waiting in the priority lane **is** persisted and replays on restore (#107) — into the lane, ahead of the inbox, on both engines (#214, #233).
> - **The chain-trip latch crosses a restart, as a `RestoredChainError`** (0.9.0, #226; 0.9.1, #243) — `chain_trips` and `last_chain_error` are snapshot fields. A restored machine reports the same count, and the error comes back as a `RestoredChainError`, which is *both* a `RestoredError` and a `RunawayChainError` — so `isinstance(interp.last_chain_error, RunawayChainError)` stays `True` across the restart. Its `.limit` / `.dropped` are `None` (JSON kept only the message); the type-independent, restart-safe guard is `interp.chain_trips > 0`. `clear_chain_error()` is still the only thing that clears it. Before #226 both reset to `0` / `None` on restore; in 0.9.0 the restored latch was a plain `RestoredError` and an `isinstance` guard went silently `False`.
> - **`strict` and event schemas apply on restore** (#214, #227) — a persisted record in `pending_events` *or* `scheduled_sends` that `send()` would have refused is dropped, reported via `on_invalid_event`, and recorded on `last_error`. Pass `from_snapshot(..., plugins=[...])` (#230) so the hook is attached *before* admission runs; otherwise the refusal is only visible on `last_error`.
...
> - **Malformed snapshots are refused, typed** — a missing key, a non-object `context`, an unknown `status`, a `running` snapshot with an empty configuration (#110), a hostile value in *any* persisted field — `version`, `status`, `history`, `actors`, `system`, `deferred`, a non-`str` payload (#146) — a pending event whose `type` is not a string (#158), or an `"error"` snapshot with no recorded error (#145) all raise `SnapshotCorruptError`. Nothing escapes `except XStateMachineError`.
> - **Configuration legality is checked on both sides (0.9.0, #142 / #143).** Legality means *exactly one active leaf per region*, not "some leaf exists": a `parallel` machine mid-transition in one region is refused with `SnapshotMidStepError` even while the other region has a leaf, and a `running` snapshot whose `configuration` lost its leaf entries (`["order"]`) is refused on restore rather than producing a live, permanently inert machine.
> - **v1 (pre-`kind`) pending events restore as *user* events** (0.9.0, #162). Classifying them by name laundered a user's `after.hours` into an engine event that `onUnhandled` / `strict` exempted. Only the init sentinel keeps system provenance. Archives written by 0.8.0 that contain engine-shaped *engine* events (an escalate, an `error.platform.*`) in the pending queue re-persist correctly as v2 on the next save; if you replay such an archive directly, re-persist it once from a 0.9.0 interpreter.
> - **Restore with the clock you mean** — `from_snapshot(..., clock=SimulatedClock())` (0.9.0, #117); without it a restored machine ran on `RealClock`, which broke deterministic replay.
...
- **`machine_hash`** — a 16-hex-character structural fingerprint of the machine (`MachineNode.structure_hash`). It is a **checksum against accidental drift, not an authentication tag** (#205): anyone with the config can compute it, and a party who controls the payload can write a consistent one. To authenticate a snapshot, sign the whole JSON outside the library and restore with `from_snapshot(minimum_version=1, expected_machine_hash=<fingerprint you recorded>)` so the payload cannot select its own level of checking. It covers states, transitions, guard/action **names**, invokes, and `after` delays — the parts of the machine that change *behavior*. It deliberately excludes `meta`, `description`, and key order, so editing a docstring or reordering a dict does not change the hash. Adding a guard, renaming a state, or changing a transition's target, on the other hand, does.
