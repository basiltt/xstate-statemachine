...
> - **The chain-trip latch DOES cross a restart** (0.9.0, #226) — `chain_trips` and `last_chain_error` are snapshot fields. A restored machine reports the same count and a `RestoredError` carrying the message; `clear_chain_error()` is still the only thing that clears it. Before #226 both reset to `0` / `None` on restore.
...
- **`chain_trips`** / **`last_chain_error`** *(#226)* — the sticky chain-trip signal (#222). Restored verbatim; the error comes back as a `RestoredError`. Absent in blobs written before 0.9.0, which restore to `0` / `None`.
